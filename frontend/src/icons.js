// Inline Lucide icon paths (https://lucide.dev, ISC license).
// Kept as raw path data so the app has zero runtime icon dependency —
// no bundler, no npm package, just inert SVG markup rendered inline.
const PATHS = {
...
export function icon(name, className = "") {
  ...
  if (!inner) return "";
  return `<svg class="icon${className ? ` ${className}` : ""}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${inner}</svg>`;
}
