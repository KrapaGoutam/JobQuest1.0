...
export async function migrateSqliteToPostgres({
  ...
  try {
    ...
    for (const table of ORDER)
      if (inspection.tables[table] && !columns.has(table))
    ...
    if (!dryRun && !validateOnly) {
      for (const table of ORDER) {
        ...
        for (const row of rows) {
          const fields = Object.keys(row).filter((field) =>
        ...
        if (columns.get(table)?.has("id"))
          await client.query(
            `SELECT setval(pg_get_serial_sequence($1,'id'), GREATEST(COALESCE((SELECT max(id) FROM ${quoted(table)}),1),1), true)`,
            [table],
          );
      }
    }
    for (const table of ORDER.filter((name) => columns.has(name))) {
      report.target_counts[table] = Number(
        (await client.query(`SELECT count(*)::int count FROM ${quoted(table)}`))
          .rows[0].count,
      );
      if (
        !dryRun &&
        (inspection.row_counts[table] || 0) > report.target_counts[table]
      )
    ...
    if (fk.rowCount)
      throw new Error("PostgreSQL contains unvalidated foreign keys");
