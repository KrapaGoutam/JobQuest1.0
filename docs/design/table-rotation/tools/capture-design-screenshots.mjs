 ...
 * approved-design-export/, and the directory's own README says to treat
 * Standalone.html as primary "unless inspection shows another exported
 * file contains a newer approved version." Inspection did: v2.dc.html is
 * the only one of the two that has a working theme toggle (Standalone.html
 * has no dark/light switching code at all - confirmed by grepping the file
 * for "theme"/"dark"/"light": zero matches), plus Undo/Redo, Clear board,
 * paused-server state, date navigation, and the "Floor team changed?"
 * clocked-in quick-add banner - all explicitly in the approved feature
 * list in ../README.md, all present in v2.dc.html, none present in
 * Standalone.html. See ../README.md's own "Primary visual reference"
 * section, which anticipates exactly this reconciliation.
 *
 ...
 *
 * Why this reuses backend/node_modules/@playwright/test directly: this
 * script's own location (docs/design/table-rotation/tools/) is not an
 * ancestor of backend/, so Node's normal bare-specifier resolution for
 * `import "@playwright/test"` won't find it there. Importing it by its
 * exact installed path reuses the repo's one existing Playwright install
